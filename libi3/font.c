...
static double pango_bg_red;
static double pango_bg_green;
static double pango_bg_blue;
...
static void draw_text_pango(const char *text, size_t text_len,
    ...
    /* Do the drawing */
    cairo_set_source_rgb(cr, pango_bg_red, pango_bg_green, pango_bg_blue);
...
void set_font_colors(xcb_gcontext_t gc, uint32_t foreground, uint32_t background) {
    ...
    switch (savedFont->type) {
        ...
        case FONT_TYPE_PANGO:
            ...
            pango_bg_red = ((background >> 16) & 0xff) / 255.0;
            pango_bg_green = ((background >> 8) & 0xff) / 255.0;
            pango_bg_blue = (background & 0xff) / 255.0;
            break;
#endif
        default:
            assert(false);
